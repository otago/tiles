	...
	generateDOM () {
        ...
        return this.state.items.map(function(item){
                        ...
                        return <div key={item.i} 
							...
							<input type="hidden" name={`Tiles[GridLayout][${item.i}][y]`} value={item.y} className="hidden form-group--no-label"/>
							<input type="hidden" name={`Tiles[GridLayout][${item.i}][w]`} value={item.w} className="hidden form-group--no-label"/>
							<input type="hidden" name={`Tiles[GridLayout][${item.i}][h]`} value={item.h} className="hidden form-group--no-label"/>
							</div>;
                      });
	}
	...
	RowsInput() {
		if(this.props.rowsenabled) {
			return (
					...
					<input type="number" name={`${this.props.name}[Rows]`} value={this.state.rows} className="numeric text tilefield__textinput" onChange={this.onChangeWidth} />
					</div>
					);
		}
	}
